#ifndef ROBOTEQFBL2360_INCLUDEDEF_H
#define ROBOTEQFBL2360_INCLUDEDEF_H
 ...
 */
class RoboteqFbl2360 : public CanDriveItf
{
public:
...
protected:
    ...
    const static int TYPE = TYPE_PROP_STEER;
    bool type_verified_;
    std::string script_version_;
    ...
    void updateWheelPos(double speed, double dt);
  ...
  //	segData seg_Data;
    ...
    */
    void sendModeCommand(std::vector<CanFrame>& msgs, int mode);
   /**
    * Sends selected PDO with two integer values.
    */
    void sendPDOdownload(CanFrame& msg, int msg_id, int data_1, int data_2);
   /**
    * Sends selected PDO with four integer values.
    */
    void sendPDOdownload(CanFrame& msg, int msg_id, int data_1, int data_2, int data_3, int data_4);
   /**
