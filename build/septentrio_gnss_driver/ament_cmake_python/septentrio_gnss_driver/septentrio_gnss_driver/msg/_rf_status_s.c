...
bool septentrio_gnss_driver__msg__rf_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("septentrio_gnss_driver.msg._rf_status.RFStatus", full_classname_dest, 46) == 0);
  }
  septentrio_gnss_driver__msg__RFStatus * ros_message = _ros_message;
  {  // header
  ...
  {  // rfband
    PyObject * field = PyObject_GetAttrString(_pymsg, "rfband");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'rfband'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!septentrio_gnss_driver__msg__RFBand__Sequence__init(&(ros_message->rfband), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create septentrio_gnss_driver__msg__RFBand__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    septentrio_gnss_driver__msg__RFBand * dest = ros_message->rfband.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!septentrio_gnss_driver__msg__rf_band__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of RFStatus */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("septentrio_gnss_driver.msg._rf_status");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "RFStatus");
  ...
  {  // rfband
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->rfband.data[i]);
      PyObject * pyitem = septentrio_gnss_driver__msg__rf_band__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    {
      int rc = PyObject_SetAttrString(_pymessage, "rfband", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
