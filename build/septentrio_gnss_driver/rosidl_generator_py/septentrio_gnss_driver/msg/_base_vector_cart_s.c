...
bool septentrio_gnss_driver__msg__base_vector_cart__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("septentrio_gnss_driver.msg._base_vector_cart.BaseVectorCart", full_classname_dest, 59) == 0);
  }
  septentrio_gnss_driver__msg__BaseVectorCart * ros_message = _ros_message;
  {  // header
  ...
  {  // vector_info_cart
    PyObject * field = PyObject_GetAttrString(_pymsg, "vector_info_cart");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'vector_info_cart'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!septentrio_gnss_driver__msg__VectorInfoCart__Sequence__init(&(ros_message->vector_info_cart), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create septentrio_gnss_driver__msg__VectorInfoCart__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    septentrio_gnss_driver__msg__VectorInfoCart * dest = ros_message->vector_info_cart.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!septentrio_gnss_driver__msg__vector_info_cart__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of BaseVectorCart */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("septentrio_gnss_driver.msg._base_vector_cart");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "BaseVectorCart");
  ...
  {  // vector_info_cart
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->vector_info_cart.data[i]);
      PyObject * pyitem = septentrio_gnss_driver__msg__vector_info_cart__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    {
      int rc = PyObject_SetAttrString(_pymessage, "vector_info_cart", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
